...
DEFAULT_ADMIN = "hackxtra"
DEFAULT_ADMIN_PASS = "hackxtra"  # change on first run!
...
# -------------------------
# Plain Text Template Parser
# -------------------------
def parse_plain_text_template(text):
    """
    Parse plain text template to extract parameters and metadata.
    Looks for {param_name} placeholders in the text.
    Returns: (pattern, metadata_dict)
    """
    ...
    unique_params = list(dict.fromkeys(params_found))
...
def extract_params_from_text(text):
    """Extract parameter names from plain text template"""
    param_pattern = r'\{([A-Za-z0-9_]+)\}'
    params_found = re.findall(param_pattern, text)
    return list(dict.fromkeys(params_found))
    ...
    while True:
        ...
        try:
            ...
            if sandbox:
                ...
                docker_cmd = ["docker", "run", "--rm", "--network", "none", "-v", "/tmp:/tmp:ro", "ubuntu:latest",
                              "bash", "-lc", shlex.join(shlex.split(cmd))]
                ...
                rc = proc.returncode
                finished = now_iso()
                log_run(template.get("id"), template.get("name"), user, params, " ".join(docker_cmd), out, err, rc,
                        started, finished)
                if on_update:
...
class App(tk.Tk):
    ...
    def init_ui(self):
        ...
        ttk.Checkbutton(actionrow, text="Run as root (sudo) [admin only]", variable=self.as_root_var).pack(side="left",
                                                                                                           padx=4)
        self.sandbox_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(actionrow, text="Sandbox (Docker) if available", variable=self.sandbox_var).pack(side="left",
                                                                                                         padx=4)
        ...
        ttk.Button(term_control_frame, text="Open Embedded Terminal", command=self.open_embedded_terminal).pack(
            side="left", padx=4)
        ttk.Button(term_control_frame, text="Kill Embedded Terminal", command=self.kill_embedded_terminal).pack(
            side="left", padx=4)
        ttk.Button(term_control_frame, text="Run in Embedded Terminal",
                   command=lambda: self.run_preview_in_terminal(embedded=True)).pack(side="left", padx=4)
        ttk.Button(term_control_frame, text="Run in External Terminal",
                   command=lambda: self.run_preview_in_terminal(embedded=False)).pack(side="left", padx=4)
    ...
    def edit_template(self):
        ...
        if not (self.selected_template.get("created_by") == self.current_user or get_user_role(
    ...
    # -------------------------
    # Plain Text Template Import
    # -------------------------
    def import_plain_text_template(self):
        """Import a plain text template with {parameter} placeholders"""
        if not self.current_user:
            messagebox.showwarning("Login required", "Please login to import templates.")
            return

        dlg = PlainTextImportDialog(self)
        self.wait_window(dlg.top)

        if not getattr(dlg, "result", None):
            return

        name, template_text, description = dlg.result

        # Parse the plain text to extract parameters
        try:
            pattern, metadata = parse_plain_text_template(template_text)

            # Show preview of extracted parameters
            param_names = [p["name"] for p in metadata["params"]]
            if param_names:
                param_info = f"Detected parameters: {', '.join(param_names)}"
            else:
                param_info = "No parameters detected"
            ...
            # Save the template
            save_template(name, pattern, metadata, description, self.current_user, approved=False)
            self.load_templates()
    ...
    def preview_command(self):
        try:
            ...
            messagebox.showerror("Params error", str(e));
            return
        if not self.selected_template:
    ...
    def run_template(self):
        if not self.current_user:
            messagebox.showwarning("Login required", "Please login before running jobs.");
            return
        if not self.selected_template:
            messagebox.showwarning("Select", "Select a template first.");
            return
        if not self.selected_template["approved"]:
            messagebox.showwarning("Not approved", "This template is not approved for execution.");
            return
        try:
            params = self.read_params() if self.param_widgets else {}
        except Exception as e:
        ...
        if as_root and get_user_role(self.current_user) != "admin":
            messagebox.showerror("Admin only", "Only admins may request root execution.");
            return
        ...
        self.output_box.insert(tk.END,
                               f"[{now_iso()}] Queued: {self.selected_template['name']} (dry={dry} as_root={as_root} sandbox={sandbox})\n")
        self.output_box.see("end")

    def schedule_template(self):
        if not self.current_user:
            messagebox.showwarning("Login required", "Please login to schedule jobs.");
            return
        if not self.selected_template:
            messagebox.showwarning("Select", "Select a template first.");
            return
        ...
        messagebox.showinfo("Scheduled",
                            f"Job scheduled (id={jid}) for {datetime.datetime.utcfromtimestamp(run_at).isoformat()}Z")
    ...
    def import_templates(self):
        ...
        try:
            ...
            for tpl in data:
                try:
                    save_template(tpl["name"], tpl["pattern"], tpl.get("metadata", {}), tpl.get("description", ""),
                                  self.current_user or "imported", approved=bool(tpl.get("approved", False)))
                    count += 1
                except Exception:
                    pass
            self.load_templates()
    ...
    def export_templates(self):
        ...
        try:
            ...
            out = []
            for t in tpls:
                out.append({"name": t["name"], "pattern": t["pattern"], "metadata": t.get("metadata", {}),
    ...
    def open_help(self):
        help_text = ("Advanced Kali Automator (Safe)\n\n"
                     ...
                     "- Use {parameter_name} placeholders in your commands\n"
                     "- Example: nmap -sS -p {ports} {target}\n"
                     "- Parameters will be automatically detected and created\n\n"
                     "Rules:\n"
    ...
    def open_embedded_terminal(self):
        ...
        if which("xterm") is None:
            messagebox.showerror("xterm missing",
                                 "xterm not found. Install with: sudo apt install xterm\nFalling back to external terminal.")
            return
        ...
        cmd = ["xterm", "-sb", "-into", str(winid), "-bg", "black", "-fg", "white", "-title", "Embedded-Terminal", "-e",
               "bash", "-l"]
        try:
            self.embedded_term_proc = subprocess.Popen(cmd)
            self.embedded_term_winid = winid
            self.after(300, lambda: self.output_box.insert("end",
                                                           f"[{now_iso()}] Embedded xterm started (pid={self.embedded_term_proc.pid}).\n"))
        except Exception as e:
        ...
        try:
            if self.embedded_term_proc and self.embedded_term_proc.poll() is None:
                ...
                self.output_box.insert("end",
                                       f"[{now_iso()}] Terminating embedded xterm (pid={self.embedded_term_proc.pid}).\n")
            else:
    ...
    def run_preview_in_terminal(self, embedded=True):
        ...
        try:
            ...
            if not messagebox.askyesno("Unsafe characters",
                                       f"Preview includes characters flagged as unsafe: {e}\nRun anyway?"):
                return
        ...
        if embedded and which("xterm"):
            ...
            try:
                ...
                xterm_cmd = ["xterm", "-sb", "-into", str(winid), "-bg", "black", "-fg", "white", "-title",
                             "Embedded-Terminal", "-e", bash_cmd]
                self.embedded_term_proc = subprocess.Popen(xterm_cmd)
                self.output_box.insert("end",
                                       f"[{now_iso()}] Running in embedded terminal (pid={self.embedded_term_proc.pid})\n")
                self.output_box.see("end")
            except Exception as e:
                messagebox.showerror("Embedded terminal failed",
                                     f"Failed to start embedded terminal: {e}\nWill try external terminal instead.")
    ...
    def run_in_external_terminal(self, full_command):
        ...
        candidates.append(
            ("x-terminal-emulator", lambda c: ["x-terminal-emulator", "-e", f"bash -lc \"{c}; exec bash\""]))
        ...
        messagebox.showerror("No terminal found",
                             "No supported external terminal emulator found (tried gnome-terminal, xterm, konsole, xfce4-terminal). Install one or use embedded xterm.")
...
class UsersDialog:
    ...
    def add_user(self):
        ...
        if role not in ("admin", "operator", "viewer"):
            messagebox.showerror("Invalid", "Role must be admin/operator/viewer");
            return
        try:
            add_user(u, p, role)
            self.refresh()
...
class SettingsDialog:
    def __init__(self, parent):
        ...
        ttk.Checkbutton(self.top, text="Enable real command execution (dangerous)", variable=self.exec_var).pack(
            anchor="w", pady=4)
...
class TemplateEditorDialog:
    def __init__(self, parent, existing):
        ...
        self.name_entry = ttk.Entry(self.top, width=70);
        self.name_entry.grid(row=0, column=1)
        ttk.Label(self.top, text="Pattern (use {param} placeholders)").grid(row=1, column=0, sticky="w")
        self.pattern_entry = ttk.Entry(self.top, width=70);
        self.pattern_entry.grid(row=1, column=1)
        ttk.Label(self.top, text="Description").grid(row=2, column=0, sticky="w")
        self.desc_entry = ttk.Entry(self.top, width=70);
        self.desc_entry.grid(row=2, column=1)
        ttk.Label(self.top, text="Metadata JSON (params list)").grid(row=3, column=0, sticky="nw")
        self.meta_text = tk.Text(self.top, width=70, height=10);
        self.meta_text.grid(row=3, column=1)
        ttk.Label(self.top,
                  text="Example metadata:\n{\"params\":[{\"name\":\"file\",\"type\":\"file\",\"label\":\"Input file\"},{\"name\":\"mode\",\"type\":\"choice\",\"options\":[\"a\",\"b\"],\"default\":\"a\"}]}").grid(
...
class PlainTextImportDialog:
    def __init__(self, parent):
        self.top = tk.Toplevel(parent)
        self.top.title("Import Plain Text Template")
        self.top.geometry("800x600")
        self.result = None

        # Template name
        ttk.Label(self.top, text="Template Name:").pack(anchor="w", padx=10, pady=(10, 2))
        self.name_entry = ttk.Entry(self.top)
        self.name_entry.pack(fill="x", padx=10, pady=(0, 10))

        # Description
        ttk.Label(self.top, text="Description:").pack(anchor="w", padx=10, pady=(0, 2))
        self.desc_entry = ttk.Entry(self.top)
        self.desc_entry.pack(fill="x", padx=10, pady=(0, 10))

        # Template text area with instructions
        ttk.Label(self.top, text="Template Command (use {parameter_name} placeholders):").pack(anchor="w", padx=10,
                                                                                               pady=(0, 2))

        # Instructions
        instruction_text = """Examples:
nmap -sS -p {ports} {target}
curl -X {method} {url} -H 'Authorization: {auth_token}'
ping -c {count} {hostname}
echo "Scanning {target} on ports {ports}" && nmap {target}"""

        instruction_label = ttk.Label(self.top, text=instruction_text, foreground="blue")
        instruction_label.pack(anchor="w", padx=10, pady=(0, 5))

        # Text area for template
        self.template_text = scrolledtext.ScrolledText(self.top, height=15, wrap=tk.WORD)
        ...
        ttk.Button(btn_frame, text="Import", command=self.on_import).pack(side="right", padx=5)
        ttk.Button(btn_frame, text="Cancel", command=self.top.destroy).pack(side="right", padx=5)

        # Load example on double-click instructions
        instruction_label.bind("<Double-1>", self.load_example)

    def load_example(self, event=None):
        """Load an example template when instructions are double-clicked"""
        example = """nmap -sS -p {ports} {target} --script {script}"""
        self.template_text.delete("1.0", tk.END)
        self.template_text.insert("1.0", example)
        self.name_entry.delete(0, tk.END)
        self.name_entry.insert(0, "Nmap TCP Scan")
        self.desc_entry.delete(0, tk.END)
        self.desc_entry.insert(0, "TCP SYN scan with custom ports and scripts")
    ...
    def on_import(self):
        ...
        if not template_text:
            messagebox.showerror("Error", "Template command text is required")
            return

        # Validate that template text doesn't contain dangerous characters
        try:
            safe_check_text(template_text)
        except ValueError as e:
            if not messagebox.askyesno("Warning", f"Template contains special characters: {e}\nImport anyway?"):
                return

        # Check for dangerous tokens
        dangerous, token = looks_dangerous(template_text)
        if dangerous:
            if not messagebox.askyesno("Warning",
                                       f"Template contains potentially dangerous token: '{token}'\nImport anyway?"):
                return

        self.result = (name, template_text, description)
        self.top.destroy()
...
class LogsDialog:
    ...
    def view(self):
        ...
        if not r:
            messagebox.showerror("Not found", "Log not found");
            return
        txt = f"Template: {r[0]}\nUser: {r[1]}\nParams: {r[2]}\nCommand: {r[3]}\n\nSTDOUT:\n{r[4]}\n\nSTDERR:\n{r[5]}\n\nRC: {r[6]}\nStarted: {r[7]}\nFinished: {r[8]}"
